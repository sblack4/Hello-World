...
/***/ function(module, exports, __webpack_require__) {
	...
	function ready() {
	    d3.json("../data/calls.json", function (error, data) {
	        if (error)
	            console.warn(error);
	        exports.fileData = fileData = getFileData(data);
	...
	var graphType;
	(function (graphType) {
	    graphType[graphType["ForceGraph"] = 0] = "ForceGraph";
	    graphType[graphType["ForceWords"] = 1] = "ForceWords";
	    graphType[graphType["WordBubbles"] = 2] = "WordBubbles";
	    graphType[graphType["PackedBubbles"] = 3] = "PackedBubbles";
	    graphType[graphType["WordCloud"] = 4] = "WordCloud";
	    graphType[graphType["Bars"] = 5] = "Bars";
	    graphType[graphType["CyclingBars"] = 6] = "CyclingBars";
	})(graphType || (graphType = {}));
	function initiateGraph(graph) {
	    switch (graph) {
	        ...
	        case graphType.CyclingBars:
	            try {
	                core_1.makeFilterDimensions(fileData.data);
	            }
	            catch (error) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	"use strict";
	var crossfilter = __webpack_require__(4);
	var bars_1 = __webpack_require__(6);
	var main_1 = __webpack_require__(1);
	var topReps = {};
	exports.topReps = topReps;
	...
	var makeFilterDimensions = function (array1) {
	    var rest = [];
	    for (var _i = 1; _i < arguments.length; _i++) {
	        rest[_i - 1] = arguments[_i];
	    }
	    topReps.filterDimensions = {};
	    var array = array1.concat.apply(array1, rest);
	    topReps.filterDimensions.filter = crossfilter(array);
	    topReps.filterDimensions.userDim = topReps.filterDimensions.filter.dimension(function (o) {
	        return (o[main_1.fileData.group]) ? o[main_1.fileData.group] : null;
	    });
	    topReps.filterDimensions.teamDim = topReps.filterDimensions.filter.dimension(function (o) {
	        return (o.TeamName) ? o.TeamName : null;
	    });
	    topReps.filterDimensions.measureDim = topReps.filterDimensions.filter.dimension(function (o) {
	        return (o[main_1.fileData.measure] > 0) ? o[main_1.fileData.measure] : null;
	    });
	};
	exports.makeFilterDimensions = makeFilterDimensions;
	topReps.getData = function (rep_range) {
	    if (rep_range === void 0) { rep_range = topReps.n_reps; }
	    var data = topReps.filterDimensions.measureDim.top(rep_range);
	    return data;
	};
	topReps.onDataChange = function (rep_range) {
	    if (rep_range === void 0) { rep_range = topReps.n_reps; }
	    var data = topReps.getData(rep_range);
	    bars_1.updateBars(data);
	};
	var cycleTeams = function () {
	    var i = 0;
	...
	exports.cycleTeams = cycleTeams;
	var cycleReps = function () {
	    bars_1.createBarsTemplate();
	    var i = 0;
	    ...
	    this.teamSwitch = function () {
	        var currentRep = i % (num_users - topReps.reps_offset);
	        var lastRep = currentRep + topReps.reps_offset;
	        var currentUsers = [];
	        for (var j = currentRep; j < lastRep; j++) {
	            allUsers[j].rank = j + 1;
	            currentUsers.push(allUsers[j]);
	        }
	        bars_1.updateBars(currentUsers);
	        i++;
	    };
	    setInterval(this.teamSwitch, topReps.CYCLE_DURATION);
	};
	exports.cycleReps = cycleReps;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var main_1 = __webpack_require__(1);
	var core_1 = __webpack_require__(3);
	var createLegend_1 = __webpack_require__(8);
	var format = d3.format(".2f");
	var svg;
	var margin = { top: 20, right: 40, bottom: 20, left: 60 };
	var w = $(document).width() - margin.left - margin.right, h = 700 - margin.top - margin.bottom;
	console.log("h", h);
	var padding_bottom = 25;
	var rank = "rank";
	var yScale = d3.scaleBand()
	    .range([padding_bottom, h]), xScale = d3.scaleLinear()
	    .range([0, w]);
	var yAxis = d3.axisLeft(yScale), xAxis = d3.axisBottom(xScale);
	...
	var createBarsTemplate = function () {
	    ...
	    svg.append("g")
	        .attr("class", "y axis");
	    createLegend_1.stringColorLegend("#legend", colorTeams, main_1.fileData.data, main_1.fileData.measure);
	    $("#xAxisLabel").text(main_1.fileData.measure);
	    d3.select('.container.xAxisContainer')
	        .style("transform", function () { return "translate(0px," + (h - 50) / 2 + "px)"; });
	};
	exports.createBarsTemplate = createBarsTemplate;
	var updateBars = function (data) {
	    var measure = main_1.fileData.measure;
	    var group = main_1.fileData.group;
	    ...
	    console.log("h", h);
	    console.log("n_reps ", core_1.topReps.n_reps);
	    var barHeight = (h / data.length);
	    console.log("data length ", data.length);
	    console.log("bar height", barHeight);
	    yScale.domain(data.map(function (d) { return d[group]; }));
	    xScale.domain([0, main_1.fileData.xAxisMax + margin.right]);
	    svg.select('.x.axis')
	        .transition().duration(core_1.topReps.TRANS_DURATION)
	        .call(xAxis)
	        .selectAll("text")
...
/***/ function(module, exports, __webpack_require__) {
	...
	"use strict";
	var d3 = __webpack_require__(2);
	var stringColorLegend = function (g, colorfunction, data, measure) {
	    ...
	    categoryNest.forEach(function (d) {
	        var sum = d3.sum(d.values, function (r) { return r[measure]; });
	...
	exports.stringColorLegend = stringColorLegend;
